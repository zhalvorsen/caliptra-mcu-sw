...
use crate::types::{RvInstr, RvMStatus, RvMemAccessType, RvMsecCfg, RvPrivMode};
...
impl<TBus: Bus> Cpu<TBus> {
    ...
    pub fn new(bus: TBus, clock: Rc<Clock>, pic: Rc<Pic>) -> Self {
        Self {
            xregs: XRegFile::new(),
...
mod tests {
    use super::*;
    use emulator_bus::{testing::FakeBus, DynamicBus, Ram, Rom, Timer};

    #[test]
    fn test_new() {
        let clock = Rc::new(Clock::new());
        let pic = Rc::new(Pic::new());
        let cpu = Cpu::new(DynamicBus::new(), clock, pic);
