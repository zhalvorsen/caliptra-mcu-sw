        ...
        match access_type {
            AccessType::Read => {
                ...
                if m.refcell {
                    ...
                    quote! {
                        #offset..=#offset_len => return emulator_bus::Bus::read(&mut self.#field_name, size, addr - #offset),
                    }
                }
            },
            AccessType::Write => {
                ...
                if m.refcell {
                    ...
                    quote! {
                        #offset..=#offset_len => return emulator_bus::Bus::write(&mut self.#field_name, size, addr - #offset, val),
                    }
                }
            },
        }
    ...
    let match_arms: Vec<_> = registers.iter().map(|reg| {
        ...
        let item_size =  || {
            if reg.ty_tokens.is_empty() {
                ...
                quote! { <#ty as emulator_bus::RegisterArray>::ITEM_SIZE }
            }
        };
        let mut array_match_pattern = || -> TokenStream {
            ...
            let len = if reg.ty_tokens.is_empty() {
                ...
                quote! { <#ty as emulator_bus::RegisterArray>::LEN }
            };
            ...
            AccessType::Read => {
                if let Some(ref read_fn) = reg.read_fn {
                    ...
                    if reg.is_array {
                        ...
                        let array_index = array_index();
                        quote! {
                            #pattern => return emulator_bus::Register::read(&self.#reg_name[#array_index], size),
                        }
                    } else {
                        quote! {
                            #offset => return emulator_bus::Register::read(&self.#reg_name, size),
                        }
                    }
                } else {
                    unreachable!();
            ...
            AccessType::Write => {
                if let Some(ref write_fn) = reg.write_fn {
                    ...
                    if reg.is_array {
                        ...
                        let array_index = array_index();
                        quote! {
                           #pattern => return emulator_bus::Register::write(&mut self.#reg_name[#array_index], size, val),
                        }
                    } else {
                        quote! {
                            #offset => return emulator_bus::Register::write(&mut self.#reg_name, size, val),
                        }
                    }
                } else {
                    unreachable!();
